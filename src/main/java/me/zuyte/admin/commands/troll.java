            ...
            if (args.length > 0) {
                ...
                if (args[0].equalsIgnoreCase("cage")) {
                    if (args.length >= 2) {
                        ...
                        if (p.getWorld().getBlockAt(x,y+2,z).getType() == Material.AIR) {
                            p.getWorld().getBlockAt(x, y+2, z).setType(Material.GLASS);
                            playerArena.addPlacedBlock(p.getWorld().getBlockAt(x, y+2, z));
                        }
                        if (p.getWorld().getBlockAt(x,y-1,z).getType() == Material.AIR) {
                            p.getWorld().getBlockAt(x, y-1, z).setType(Material.GLASS);
                            playerArena.addPlacedBlock(p.getWorld().getBlockAt(x, y-1, z));
                        }
                        if (p.getWorld().getBlockAt(x,y,z+1).getType() == Material.AIR) {
                            p.getWorld().getBlockAt(x, y, z+1).setType(Material.GLASS);
                            playerArena.addPlacedBlock(p.getWorld().getBlockAt(x, y, z+1));
                        }
                        if (p.getWorld().getBlockAt(x,y,z-1).getType() == Material.AIR) {
                            p.getWorld().getBlockAt(x, y, z-1).setType(Material.GLASS);
                            playerArena.addPlacedBlock(p.getWorld().getBlockAt(x,y,z-1));
                        }
                        if (p.getWorld().getBlockAt(x+1,y,z).getType() == Material.AIR) {
                            p.getWorld().getBlockAt(x+1, y, z).setType(Material.GLASS);
                            playerArena.addPlacedBlock(p.getWorld().getBlockAt(x+1, y, z));
                        }
                        if (p.getWorld().getBlockAt(x-1,y,z).getType() == Material.AIR) {
                            p.getWorld().getBlockAt(x-1, y, z).setType(Material.GLASS);
                            playerArena.addPlacedBlock(p.getWorld().getBlockAt(x-1, y, z));
                        ...
                        // above layer
                        if (p.getWorld().getBlockAt(x, y+1, z+1).getType() == Material.AIR) {
                            p.getWorld().getBlockAt(x, y+1, z+1).setType(Material.GLASS);
                            playerArena.addPlacedBlock(p.getWorld().getBlockAt(x, y+1, z+1));
                        }
                        if (p.getWorld().getBlockAt(x, y+1, z-1).getType() == Material.AIR) {
                            p.getWorld().getBlockAt(x, y+1, z-1).setType(Material.GLASS);
                            playerArena.addPlacedBlock(p.getWorld().getBlockAt(x, y+1, z-1));
                        }
                        if (p.getWorld().getBlockAt(x+1, y+1, z).getType() == Material.AIR) {
                            p.getWorld().getBlockAt(x+1, y+1, z).setType(Material.GLASS);
                            playerArena.addPlacedBlock(p.getWorld().getBlockAt(x+1, y+1, z));
                        }
                        if (p.getWorld().getBlockAt(x-1, y+1, z).getType() == Material.AIR) {
                            p.getWorld().getBlockAt(x-1, y+1, z).setType(Material.GLASS);
                            playerArena.addPlacedBlock(p.getWorld().getBlockAt(x-1, y+1, z));
                ...
                if (args[0].equalsIgnoreCase("ghast")) {
                    if (args.length >= 2) {
                        ...
                        Entity ghast = player.getWorld().spawnEntity(player.getLocation(), EntityType.GHAST);
                        LivingEntity livingGhast = (LivingEntity) ghast;
                        Creature creatureBlaze = (Creature) livingGhast;
                        creatureBlaze.setTarget(player);
                        p.sendMessage(ChatColor.RED + "Summoned Ghast targeting " + args[1]);
                        return true;
                    }
                }
                if (args[0].equalsIgnoreCase("blaze")) {
                    if (args.length >= 2) {
                        ...
                        p.sendMessage(ChatColor.RED + "Summoned Blaze targeting " + args[1]);
                        return true;
                    }
                }
                if (args[0].equalsIgnoreCase("toystick")) {
                    if (args.length >= 2) {
                        ...
                        p.getInventory().addItem(toyStick);
